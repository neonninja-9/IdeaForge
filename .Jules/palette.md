...
## 2026-08-14 - Missing ARIA label for standalone input in Footer
**Learning:** Standalone inputs without visual labels (like the newsletter signup in the Footer) need explicit `aria-label` attributes for screen reader support.
**Action:** Add `aria-label` to all standalone inputs (e.g., search, newsletter) where visual labels are omitted by design.
