...
export interface OptionWheelProps {
  ...
  /** Called only when a user explicitly clicks/taps an item (not on scroll or drag). */
...
/**
 * Position every item along the curved arc at the given scroll position.
 * This is a pure function so it can be called from both useLayoutEffect
 * (synchronous, before paint) and the rAF animation loop.
 */
function positionItems(
  ...
  for (let i = 0; i < n; i++) {
    ...
    el.style.color = dist < 0.5 ? activeColor : textColor;
    el.style.fontWeight = dist < 0.5 ? '600' : '400';
  }
}
...
const OptionWheel = ({
  ...
  // Keep refs in sync with latest props so the rAF closure always reads current values.
  ...
  // ── Synchronous initial layout ───────────────────────────────────────
  // Runs BEFORE the browser paints, so items are never seen in their
  // unstyled "all piled at top:50%" state.
  useLayoutEffect(() => {
  ...
  }, [defaultSelected, items.length, fontSize, spacing, curve, tilt, blurAmount, fade, minOpacity, side, loop, activeColor, textColor, remPx]);
  ...
  const runFrame = useCallback((now: number) => {
    ...
    positionItems(itemRefs.current, c, next, activeColorRef.current, textColorRef.current);
  ...
  const applyTarget = useCallback(
    (value: number, snap: boolean) => {
      ...
      if (idx !== selectedRef.current) {
        ...
        onChangeRef.current?.(idx, c.items[idx]);
        playTick();
      }
      startLoop();
    },
  ...
  const handleItemClick = useCallback(
    (index: number) => {
      ...
      onItemClickRef.current?.(index, c.items[index]);
    },
    [applyTarget]
  );
    ...
    >
      {items.map((label, index) => (
        <div
          ...
          className={`absolute top-1/2 cursor-pointer whitespace-nowrap leading-none will-change-[transform,opacity,filter] [font-size:var(--ow-font-size)] ${
