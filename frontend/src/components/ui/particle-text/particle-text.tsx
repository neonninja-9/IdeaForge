...
export const ParticleText = ({
  ...
  const hasGatheredRef = useRef<boolean>(false);
  ...
  useEffect(() => {
    ...
    const sampleText = async (): Promise<void> => {
      ...
      const alreadyGathered = hasGatheredRef.current && playOnce;
      ...
      particles = selected.map((target, index) => {
        ...
        const distance = (reducedMotion || alreadyGathered ? 0 : scatter) * (0.35 + depth * 0.75);
        const startX = target.x + (alreadyGathered ? 0 : Math.cos(angle) * distance + (seed - 0.5) * scatter * 0.45);
        const startY = target.y + (alreadyGathered ? 0 : Math.sin(angle) * distance + (depth - 0.9) * scatter * 0.45);

        return {
          x: reducedMotion || alreadyGathered ? target.x : startX,
          y: reducedMotion || alreadyGathered ? target.y : startY,
          startX: alreadyGathered ? target.x : startX,
          startY: alreadyGathered ? target.y : startY,
          targetX: target.x,
          targetY: target.y,
      ...
      if (reducedMotion || alreadyGathered) {
        particles.forEach((particle) => {
