...
import { submitIdeaAction } from "@/app/submit/actions";
import { Category, Tag } from "@prisma/client";

export default function SubmitIdeaForm({ categories, tags }: { categories: Category[], tags: Tag[] }) {
  const [state, dispatch, isPending] = useActionState(
    (prevState: unknown, formData: FormData) => submitIdeaAction(prevState, formData),
    undefined
  );
      ...
      <form action={dispatch} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <select
              id="difficulty"
              name="difficulty"
              defaultValue={(state?.data?.difficulty as string) || ""}
              className="w-full px-4 py-3 bg-surface border border-edge rounded-xl focus:outline-none focus:ring-2 focus:ring-vivid/50 appearance-none"
            >
            ...
            {tags.map(t => (
              <label key={t.id} className="flex items-center gap-2 bg-white px-3 py-2 rounded-lg border border-edge cursor-pointer hover:border-vivid transition-colors">
                <input
                  ...
                  name="tags"
                  value={t.id}
                  defaultChecked={Array.isArray(state?.data?.tags) && state.data.tags.includes(t.id)}
        ...
        <div>
          ...
          <textarea
            ...
            name="problem"
            rows={4}
            defaultValue={(state?.data?.problem as string) || ""}
        ...
        <div>
          ...
          <textarea
            ...
            name="solution"
            rows={4}
            defaultValue={(state?.data?.solution as string) || ""}
            className="w-full px-4 py-3 bg-surface border border-edge rounded-xl focus:outline-none focus:ring-2 focus:ring-vivid/50 resize-none"
        ...
        <div>
          ...
          <textarea
            ...
            name="impact"
            rows={2}
            defaultValue={(state?.data?.impact as string) || ""}
            className="w-full px-4 py-3 bg-surface border border-edge rounded-xl focus:outline-none focus:ring-2 focus:ring-vivid/50 resize-none"
            placeholder="What positive change will this bring?"
          ></textarea>
