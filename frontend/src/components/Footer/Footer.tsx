    ...
    <footer className="bg-surface-dark text-white pt-16 sm:pt-20 pb-10 px-5 sm:px-6 lg:px-10">
      <div className="max-w-7xl mx-auto">
        <div className="grid gap-10 mb-14 sm:grid-cols-2 sm:gap-12 sm:mb-20 lg:grid-cols-4">
          <div>
            <Link to="/" className="text-2xl font-black tracking-tight mb-4 block hover:text-vivid-light transition-colors">IdeaForge</Link>
            <p className="text-sm text-fg-muted leading-relaxed mb-5">
              Editorial Storytelling &amp; Digital Depth. A new standard for professional idea management.
            </p>
            <div className="flex gap-3">
              <a href="#" className="w-9 h-9 rounded-lg bg-surface-dark-card border border-edge-dark flex items-center justify-center text-fg-muted hover:text-white hover:border-vivid transition-all duration-300" aria-label="Twitter">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" /></svg>
              </a>
              <a href="https://github.com/neonninja-9/IdeaForge" target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-lg bg-surface-dark-card border border-edge-dark flex items-center justify-center text-fg-muted hover:text-white hover:border-vivid transition-all duration-300" aria-label="GitHub">
              ...
              <a href="#" className="w-9 h-9 rounded-lg bg-surface-dark-card border border-edge-dark flex items-center justify-center text-fg-muted hover:text-white hover:border-vivid transition-all duration-300" aria-label="LinkedIn">
          ...
          {Object.entries(footerLinks).map(([category, links]) => (
            <div key={category}>
              <h4 className="text-[11px] font-mono uppercase tracking-[0.25em] text-fg-muted mb-5">{category}</h4>
              <ul className="space-y-3">
                {links.map((link) => (
                  <li key={link.label}>
                    {link.to ? (
                      <Link to={link.to} className="text-sm text-fg-muted hover:text-white transition-colors duration-300">{link.label}</Link>
                    ) : (
                      <a
                        href={link.href}
                        {...(link.href?.startsWith("http") ? { target: "_blank", rel: "noopener noreferrer" } : {})}
                        className="text-sm text-fg-muted hover:text-white transition-colors duration-300"
                      >
                        {link.label}
                      </a>
                    )}
