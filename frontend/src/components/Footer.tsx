const footerLinks = {
  Product: ["The Forge", "Integrations", "API Access", "Pricing"],
  Developer: ["Documentation", "Status", "Open Source", "GitHub"],
};
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-12 mb-20">
          <div>
            ...
            <div className="flex gap-3">
              ...
              <a href="#" className="w-9 h-9 rounded-lg bg-surface-dark-card border border-edge-dark flex items-center justify-center text-fg-muted hover:text-white hover:border-vivid transition-all duration-300" aria-label="GitHub">
          ...
          {Object.entries(footerLinks).map(([category, links]) => (
            <div key={category}>
              ...
              <ul className="space-y-3">
                {links.map((link) => (
                  <li key={link}>
                    <a href="#" className="text-sm text-fg-muted hover:text-white transition-colors duration-300">{link}</a>
                  </li>
                ))}
              </ul>
            </div>
