...
export default function SocialProof() {
  ...
  return (
    <section ref={ref} id="proof" className="py-20 sm:py-28 lg:py-36 px-5 sm:px-6 lg:px-10 bg-surface-alt dot-grid">
      <div className="max-w-7xl mx-auto">
        <div className={`mb-12 sm:mb-20 transition-all duration-700 ${inView ? "translate-y-0" : "translate-y-6"}`}>
          ...
          <h2 className="text-4xl md:text-6xl font-black tracking-normal leading-[1.05] text-fg max-w-2xl">
        ...
        <div className="grid gap-10 lg:grid-cols-2 lg:gap-16">
          <div className="space-y-10">
            <div className={`flex flex-col gap-6 sm:flex-row sm:gap-10 lg:gap-14 transition-all duration-700 ${inView ? "opacity-100 translate-y-0" : "opacity-60 translate-y-4"}`} style={{ transitionDelay: "200ms" }}>
              <div>
                <p className="text-5xl sm:text-6xl font-black text-vivid tracking-normal">5×</p>
                <p className="text-[11px] text-fg-light mt-2 uppercase tracking-[0.15em] font-medium">Faster Delivery</p>
              </div>
              <div className="h-px w-full bg-edge sm:h-auto sm:w-px" />
              <div>
                <p className="text-5xl sm:text-6xl font-black text-fg tracking-normal">94%</p>
            ...
            <div className={`relative transition-all duration-700 ${inView ? "opacity-100 translate-y-0" : "opacity-60 translate-y-4"}`} style={{ transitionDelay: "400ms" }}>
              <div className="relative overflow-hidden min-h-[260px] sm:min-h-[200px] lg:min-h-[180px]">
                {testimonials.map((t, i) => (
                  <div key={i} className={`absolute inset-0 transition-all duration-700 ${i === activeTestimonial ? "opacity-100 translate-x-0" : "opacity-0 translate-x-8"}`}>
                    <blockquote className="border-l-3 border-vivid pl-5 sm:pl-6">
                      <p className="text-base italic text-fg leading-relaxed mb-5 sm:text-lg">&ldquo;{t.quote}&rdquo;</p>
                    </blockquote>
                    <div className="flex items-center gap-3 pl-5 sm:pl-6">
                      <div className="w-10 h-10 shrink-0 rounded-full bg-fg flex items-center justify-center text-white text-sm font-bold">{t.initials}</div>
                      <div>
              ...
              <div className="flex gap-2 mt-6 pl-5 sm:pl-6">
          ...
          <div className="grid gap-5">
            {caseStudies.map((study, i) => (
              <div key={i} className={`group relative bg-white rounded-2xl border border-edge p-6 sm:p-8 hover:shadow-xl transition-all duration-500 cursor-pointer ${inView ? "opacity-100 translate-x-0" : "opacity-0 translate-x-12"}`} style={{ transitionDelay: inView ? `${300 + i * 150}ms` : "0ms" }}>
