...
export default function AIStudioPage() {
  ...
  async function send(value = message) {
    ...
    if (user) {
      ...
      try {
        ...
        setMessages((prev) => [
          ...prev,
          {
            ...
            conversation: convId,
            role: "assistant",
            text: "I couldn’t reach the IdeaForge assistant. Please try again in a moment.",
            createdAt: new Date().toISOString(),
          },
        ]);
      } finally {
      ...
      try {
        ...
        setMessages((prev) => [
          ...prev,
          { _id: (Date.now() + 1).toString(), conversation: "", role: "assistant", text: "I couldn’t reach the IdeaForge assistant. Please try again in a moment.", createdAt: new Date().toISOString() },
        ]);
      } finally {
        setThinking(false);
      }
  ...
  return (
    <div className="min-h-[calc(100vh-76px)] bg-[#fafaf8] px-5 py-7 sm:px-8 sm:py-10 xl:px-12">
      <main className="mx-auto grid max-w-[1180px] gap-7 xl:grid-cols-[300px_minmax(0,1fr)]">
        <aside className="rounded-[28px] border border-slate-100 bg-white p-6 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between">
              ...
              {user && (
                <button
                  ...
                  className="inline-flex items-center gap-1 text-xs font-semibold text-indigo-600 hover:text-indigo-700 bg-indigo-50 hover:bg-indigo-100 px-3 py-2 rounded-xl transition"
                >
                  <Plus size={14} /> New Chat
                </button>
              )}
            </div>
            <p className="mt-6 text-sm font-semibold text-violet-600">AI STUDIO</p>
            <h1 className="font-heading mt-1 text-2xl font-bold text-slate-900">Your creative cofounder.</h1>
            <p className="mt-3 text-sm leading-6 text-slate-500">Explore your ideas interactively.</p>

            {user && conversations.length > 0 && (
              <div className="mt-6">
                <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">History</p>
            ...
            <div className="mt-6 space-y-2">
              <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Prompts</p>
              {prompts.map((prompt) => (
                <button
                  key={prompt}
                  onClick={() => send(prompt)}
                  className="flex min-h-10 w-full items-center justify-between rounded-xl bg-slate-50 px-3 text-left text-xs font-medium text-slate-600 transition hover:bg-violet-50 hover:text-violet-700"
                >
                  <span>{prompt}</span>
                  <ArrowUp size={14} />
                </button>
          ...
          <p className="mt-6 rounded-2xl bg-amber-50 p-3 text-xs leading-5 text-amber-800">
            Responses are delivered by the secured IdeaForge API & powered by Gemini.
          </p>
        </aside>

        <section className="flex min-h-[620px] flex-col overflow-hidden rounded-[28px] border border-indigo-100 bg-white shadow-[0_18px_50px_-32px_rgba(79,70,229,.4)]">
          <header className="flex items-center gap-3 border-b border-slate-100 bg-gradient-to-r from-indigo-50 to-violet-50 px-6 py-5">
            <span className="grid size-10 place-items-center rounded-2xl bg-white text-indigo-600 shadow-sm">
              <Bot size={20} />
            </span>
            <div>
              <h2 className="font-heading text-lg font-bold text-slate-900">IdeaForge AI</h2>
              <p className="flex items-center gap-1 text-xs text-emerald-600">
                <span className="size-1.5 rounded-full bg-emerald-500" /> Ready to explore
              </p>
            </div>
          </header>
                ...
                <div key={item._id || item.id} className={`flex ${item.role === "user" ? "justify-end" : "justify-start"}`}>
                  <div
                    className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-6 ${
                      item.role === "user" ? "bg-indigo-600 text-white" : "bg-slate-50 text-slate-600"
                    }`}
                  >
                    {item.role === "assistant" && <Lightbulb size={15} className="mb-2 text-violet-500" />}
                    {item.text}
                  </div>
                </div>
              ))
          ...
          >
            <div className="flex items-end gap-3 rounded-2xl border border-slate-200 bg-[#fcfcfd] p-2 focus-within:border-indigo-300 focus-within:ring-4 focus-within:ring-indigo-50">
              <textarea
                value={message}
                onChange={(event) => setMessage(event.target.value)}
                rows={2}
                placeholder="Ask anything about your idea..."
                className="min-h-11 flex-1 resize-none bg-transparent px-2 py-2 text-sm text-slate-700 outline-none placeholder:text-slate-400"
              />
              <button
                type="submit"
