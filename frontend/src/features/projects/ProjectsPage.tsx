...
const canvasBlocks: CanvasBlock[] = [
  ...
  { title: "Revenue", prompt: "How could the value become sustainable?", gradient: "from-[#FFFBEB] to-white", tone: "text-[#A16207]", suggestion: "Choose a simple value exchange: subscription, transaction, or team plan." },
  ...
  { title: "Metrics", prompt: "What signal proves it is working?", gradient: "from-[#FFFBEB] to-white", tone: "text-[#A16207]", suggestion: "Pick one behavior that tells you a user received meaningful value." },
    ...
    <div className="min-h-[calc(100vh-76px)] bg-[var(--background)] dark:bg-transparent px-5 py-7 sm:px-8 sm:py-10 xl:px-12 transition-colors duration-500">
      <main className="mx-auto max-w-[1440px]">
        <header className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
          <div>
            <p className="text-sm font-semibold text-[#A16207] dark:text-[#A16207]">PROJECT CANVAS</p>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button onClick={handleExportCanvas} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#1C1917] px-4 text-sm font-semibold text-slate-700 dark:text-slate-300 shadow-sm transition hover:bg-slate-50 dark:hover:bg-white/5">
              <Download size={17} /> Export Canvas
            </button>
            <button onClick={startNewCanvas} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#A16207] px-5 text-sm font-semibold text-white shadow-lg shadow-[#A162071a] dark:shadow-none transition hover:-translate-y-0.5 hover:bg-[#854D0E]">
        ...
        <section className="mt-8 grid gap-4 md:grid-cols-3">
          <article className="rounded-3xl border border-[#ededed] dark:border-white/10 bg-gradient-to-br from-[#A16207] to-[#EAB308] p-6 text-white shadow-lg shadow-[#A162071a] dark:shadow-none transition-colors duration-500">
        ...
        <section className="mt-10 rounded-[30px] border border-slate-100 dark:border-white/5 bg-white dark:bg-[#1C1917] p-5 shadow-[0_18px_50px_-32px_rgba(15,23,42,.3)] dark:shadow-none sm:p-7 transition-colors duration-500">
          <div className="flex flex-col gap-4 border-b border-slate-100 dark:border-white/5 pb-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold text-[#A16207] dark:text-[#A16207]">STARTUP CANVAS</p><h2 className="font-heading mt-1 text-2xl font-bold text-slate-900 dark:text-white">A working model, one block at a time</h2></div><span className="inline-flex items-center gap-2 rounded-full bg-emerald-50 dark:bg-emerald-500/10 px-3 py-1.5 text-xs font-semibold text-emerald-700 dark:text-emerald-400"><CircleCheck size={14} /> Synced to cloud</span></div>
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {canvasBlocks.map((block) => {
              const isExpanded = expanded === block.title;
              return <article key={block.title} className={`group rounded-3xl border p-5 transition-all duration-300 ${selected === block.title ? "border-[#FDE047] ring-4 ring-[#FEF3C7] dark:border-white/10 dark:ring-white/5" : "border-slate-100 dark:border-white/5 hover:border-[#FDE047] dark:hover:border-white/15"} bg-gradient-to-br ${block.gradient} dark:from-[#292524] dark:to-[#292524]`}>
                ...
                {isExpanded && <textarea value={notes[block.title] || ""} onChange={(event) => updateNote(block.title, event.target.value)} className="mt-4 w-full resize-none rounded-xl border border-white dark:border-white/10 bg-white/80 dark:bg-[#1C1917]/80 p-3 text-sm text-slate-600 dark:text-slate-300 outline-none focus:ring-2 focus:ring-indigo-200 dark:focus:ring-[#FEF3C7]0/50 transition-colors" rows={4} placeholder="Add your working note..." />}
                {!isExpanded && notes[block.title]?.trim() && <p className="mt-4 line-clamp-2 rounded-xl bg-white/65 dark:bg-[#1C1917]/65 p-3 text-sm leading-6 text-slate-600 dark:text-slate-400 transition-colors">{notes[block.title]}</p>}
              </article>;
            })}
          </div>
          <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-slate-50 dark:bg-[#292524] px-4 py-3 text-sm text-slate-500 dark:text-slate-400 transition-colors"><span>Selected block: <strong className="font-semibold text-slate-700 dark:text-slate-300">{selected}</strong></span><div className="flex items-center gap-4"><span className={saveState === "error" ? "text-rose-600 dark:text-rose-400" : "text-emerald-700 dark:text-emerald-400"}>{saveState === "saving" ? "Saving…" : saveState === "error" ? "Could not save changes" : "Saved to your account"}</span><button onClick={addSuggestion} className="inline-flex min-h-11 items-center gap-1 font-semibold text-[#A16207] dark:text-[#A16207] hover:text-[#854D0E] dark:hover:text-indigo-300">Add a starter prompt <ArrowUpRight size={16} /></button></div></div>
        </section>
      </main>
    </div>
  );
