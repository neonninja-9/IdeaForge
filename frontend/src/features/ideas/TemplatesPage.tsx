...
const templates = [
  ...
  { title: "MVP experiment", description: "Turn an early thought into a focused test with one audience and one measurable signal.", prompt: "Help me plan a small MVP experiment for ", icon: Rocket, tone: "bg-[#FEF3C7] dark:bg-white/5 text-[#A16207] dark:text-[#A16207]" },
    ...
    <div className="min-h-[calc(100vh-76px)] bg-[var(--background)] dark:bg-transparent px-5 py-7 sm:px-8 sm:py-10 xl:px-12 transition-colors duration-500">
      <main className="mx-auto max-w-6xl">
        <header className="max-w-2xl">
          <p className="text-sm font-semibold text-[#A16207] dark:text-[#A16207]">TEMPLATES</p>
        ...
        <section className="mt-9 grid gap-5 md:grid-cols-3">
          {templates.map(({ title, description, prompt, icon: Icon, tone }) => (
            <article key={title} className="flex min-h-72 flex-col rounded-[28px] border border-slate-100 dark:border-white/5 bg-white dark:bg-[#1C1917] p-6 shadow-sm dark:shadow-none transition-colors">
              ...
              <Link to={`/submit?prompt=${encodeURIComponent(prompt)}`} className="mt-7 inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-[#A16207] dark:text-[#A16207] hover:text-[#854D0E] dark:hover:text-indigo-300">
        ...
        <section className="mt-8 flex flex-col gap-4 rounded-[28px] border border-[#ededed] dark:border-[#CA8A04]/20 bg-gradient-to-r from-[#FFFBEB] to-[#FEF3C7] dark:from-[#292524/20] dark:to-[#292524/20] p-6 sm:flex-row sm:items-center sm:justify-between transition-colors">
          <div className="flex items-center gap-4">
            <span className="grid size-11 place-items-center rounded-2xl bg-white dark:bg-[#1C1917] text-[#A16207] dark:text-[#CA8A04] shadow-sm dark:shadow-none">
              <Bot size={20} />
            </span>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="font-heading font-bold text-slate-900 dark:text-white">AI Studio Co-pilot</h2>
                <span className="rounded-full bg-[#fff0c2] dark:bg-[#A16207]/20 px-2 py-0.5 text-[10px] font-semibold text-[#854D0E] dark:text-[#CA8A04]">Live demo</span>
              </div>
        ...
        <section className="mt-6 rounded-[28px] border border-slate-100 dark:border-white/5 bg-white dark:bg-[#1C1917] p-6 shadow-sm dark:shadow-none transition-colors">
