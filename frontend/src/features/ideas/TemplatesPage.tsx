...
const templates = [
  ...
  { title: "MVP experiment", description: "Turn an early thought into a focused test with one audience and one measurable signal.", prompt: "Help me plan a small MVP experiment for ", icon: Rocket, tone: "bg-[#fff8e0] dark:bg-white/5 text-[#fa520f] dark:text-[#fa520f]" },
    ...
    <div className="min-h-[calc(100vh-76px)] bg-[var(--background)] dark:bg-transparent px-5 py-7 sm:px-8 sm:py-10 xl:px-12 transition-colors duration-500">
      <main className="mx-auto max-w-6xl">
        <header className="max-w-2xl">
          <p className="text-sm font-semibold text-[#fa520f] dark:text-[#fa520f]">TEMPLATES</p>
        ...
        <section className="mt-9 grid gap-5 md:grid-cols-3">
          {templates.map(({ title, description, prompt, icon: Icon, tone }) => (
            <article key={title} className="flex min-h-72 flex-col rounded-[28px] border border-slate-100 dark:border-white/5 bg-white dark:bg-[#120F17] p-6 shadow-sm dark:shadow-none transition-colors">
              ...
              <Link to={`/submit?prompt=${encodeURIComponent(prompt)}`} className="mt-7 inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-[#fa520f] dark:text-[#fa520f] hover:text-[#cc3a05] dark:hover:text-indigo-300">
