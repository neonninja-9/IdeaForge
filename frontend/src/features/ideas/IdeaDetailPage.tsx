        ...
        <header className="mt-6">
          ...
          <div className="mt-5 flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-1">
              {isAuthor && (
                <>
                  <Link to={`/edit-idea/${id}`} aria-label="Edit idea" className="grid size-11 sm:size-9 place-items-center rounded-lg text-slate-400 dark:text-slate-500 transition hover:bg-slate-100 dark:hover:bg-white/5 hover:text-slate-700 dark:hover:text-white focus-visible:ring-2 focus-visible:outline-none" title="Edit">
                    <Edit3 size={16} />
                  </Link>
                  <button onClick={handleDeleteIdea} aria-label="Delete idea" className="grid size-11 sm:size-9 place-items-center rounded-lg text-slate-400 dark:text-slate-500 transition hover:bg-rose-50 dark:hover:bg-rose-500/10 hover:text-rose-500 dark:hover:text-rose-400 focus-visible:ring-2 focus-visible:outline-none" title="Delete">
                    <Trash2 size={16} />
                  </button>
                </>
              )}
              <button onClick={handleExportMarkdown} aria-label="Export Markdown" className="grid size-11 sm:size-9 place-items-center rounded-lg text-slate-400 dark:text-slate-500 transition hover:bg-slate-100 dark:hover:bg-white/5 hover:text-slate-700 dark:hover:text-white focus-visible:ring-2 focus-visible:outline-none" title="Export Markdown">
                <Download size={16} />
              </button>
              <button onClick={handleFavorite} aria-label={isFavorite ? "Remove from favorites" : "Add to favorites"} className={`grid size-11 sm:size-9 place-items-center rounded-lg transition focus-visible:ring-2 focus-visible:outline-none ${isFavorite ? "text-rose-500 bg-rose-50 dark:bg-rose-500/10" : "text-slate-400 dark:text-slate-500 hover:bg-slate-100 dark:hover:bg-white/5 hover:text-rose-500"}`} title={isFavorite ? "Remove from favorites" : "Add to favorites"}>
                <Heart size={16} className={isFavorite ? "fill-current" : ""} />
              </button>
              <button onClick={handleShare} aria-label="Share" className="grid size-11 sm:size-9 place-items-center rounded-lg text-slate-400 dark:text-slate-500 transition hover:bg-slate-100 dark:hover:bg-white/5 hover:text-slate-700 dark:hover:text-white focus-visible:ring-2 focus-visible:outline-none" title="Share">
                <Share2 size={16} />
              </button>
                  ...
                  return (
                    <article key={comment.id || comment._id} className="rounded-xl bg-slate-50 dark:bg-white/[0.03] p-4">
                      <div className="flex items-center gap-2.5">
                        ...
                        {isMyComment && (
                          <button onClick={() => handleDeleteComment(comment.id || comment._id)} aria-label="Delete comment" className="min-h-[44px] sm:min-h-0 text-[10px] font-medium text-rose-400 hover:text-rose-600 focus-visible:ring-2 focus-visible:outline-none rounded px-1">Delete</button>
                        )}
                      </div>
